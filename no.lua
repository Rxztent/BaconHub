...
msgdone.OnClientEvent:Connect(function(msgdata)
	...
	if message == '.bet' then
		chat('💸 You need to specify an amount! | .bet amount 💸')
		return
	end
	if string.match(message, '.bet ') then
		local slotValx = string.gsub(message, '.bet ', '')
		if not tonumber(slotValx) or tonumber(slotValx) < 0 then
			chat('💸 Enter a valid amount! | .bet amount 💸')
			return
		end
		local slotVal = tonumber(slotValx)
		if bal == 0 then
			chat('💸 Your balance is 0! You cannot bet! 💸')
			return
		end
		if bal < slotVal then
...
task.spawn(function()
	while true do
		boothText:FireServer('<stroke color="#444444" thickness="1"><font size="9"><font color= "#58eb0e"><font face="DenkOne">💸         Win Bobux!         💸 ------💸-Golden Casino-💸  💸          .help to play        💸</font></font></font></stroke>', "booth")
		task.wait(3)
		boothText:FireServer('<stroke color="#444444" thickness="1"><font size="9"><font color= "#58eb0e"><font face="DenkOne">💸         Win Bobux!         💸 -----💸-Golden Casino-💸-  💸          .help to play        💸</font></font></font></stroke>', "booth")
		task.wait(3)
		boothText:FireServer('<stroke color="#444444" thickness="1"><font size="9"><font color= "#58eb0e"><font face="DenkOne">💸         Win Bobux!         💸 ----💸-Golden Casino-💸--  💸          .help to play        💸</font></font></font></stroke>', "booth")
